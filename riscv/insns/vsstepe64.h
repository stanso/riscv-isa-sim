...
for (int i = 0; i < 12; i++)
{
    ...
    if (cur_addr_reg_num != 0)
    {
        ...
        if (!is_store)
        {
            ...
            // output number of valid element count in the output stream vector
            reg_t valid_elem_cnt = 0;
            for (reg_t j = 0; j < elt_per_reg; ++j)
            {
                if (P.VU.elt<type_sew_t<e64>::type>(val_reg_num, j) != UINT64_MAX)
                    valid_elem_cnt ++;
            }
            WRITE_RD(valid_elem_cnt);
            // advance address
            WRITE_REG(cur_addr_reg_num, sext_xlen(baseAddr + 8 * (adv_elem_cnt < stream_len_left ? adv_elem_cnt : stream_len_left)));
        ...
        else
        {
            ...
            // output total number of elements written to the memory on store case
            WRITE_RD(stream_len_left);
